...
def speak_text(text, language, gender):
    global engine  # Use the global pyttsx3 engine
    gender = genders
    if gender == 'male':
        # Set voice to male (this might vary depending on available voices)
        ...
        if male_voice is None:
            male_voice = voices[0]  # fallback to the default voice if no male voice found
...
#     # Example variables
#     gender = 'female'  # 'male' and 'female' options can be added if needed
#     text = 'Hello, how are you?'
#     language = 'bn'  # Spanish language code
    
#     # Speak the text in the specified language
#     speak_text(text, language, gender)
#--------------------------------------------------
genders = None
        ...
        while cap.isOpened():
            ...
            if results.right_hand_landmarks and results.face_landmarks:
                ...
                if len(sequence) == 30:
                    ...
                    if predicted_action == 'hello':
                        cap.release()
                        cv2.destroyAllWindows()
                        speak_text(action_text, language, genders)  # Use speak_text here
                        sign_model()
                        continue  
                    ...
                    if res[np.argmax(res)] > threshold:
                        if len(sentence) > 0:
                            if predicted_action != sentence[-1]:
                                ...
                                speak_text(predicted_action, language, genders)  # Use speak_text here
                        else:
                            sentence.append(predicted_action)
                            speak_text(predicted_action, language, genders)  # Use speak_text here
        ...
        if results.multi_hand_landmarks:
            ...
            if pause_start_time is None:
                ...
                if collected_letters:
                    ...
                    speak_text(displayed_word, language, genders)  # Use speak_text here

                    if displayed_word == "A":
                        cap.release()
                        cv2.destroyAllWindows()
                        speak_text(sign_text, language, genders)  # Use speak_text here
                        action_model()
